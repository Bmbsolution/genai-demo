...
describe("apiFetch", () => {
  ...
  it("unwraps the error envelope into a typed ApiError on non-401 failures", async () => {
    ...
    const error = await apiFetch("/api/v1/events").catch((e: unknown) => e);
  ...
  it("refreshes the token on a 401 and retries the request once", async () => {
    ...
    const result = await apiFetch<{ data: { ok: boolean } }>("/api/v1/events");
  ...
  it("does not attempt a refresh when there is no refresh token", async () => {
    ...
    await apiFetch("/api/v1/events").catch(() => undefined);
  ...
  it("serializes the request body for writes", async () => {
    ...
    await apiFetch("/api/v1/events", { method: "POST", body: { name: "x" } });
  ...
  it("returns undefined for a 204 No Content response", async () => {
    ...
    const result = await apiFetch<undefined>("/api/v1/events/1", { method: "DELETE" });
